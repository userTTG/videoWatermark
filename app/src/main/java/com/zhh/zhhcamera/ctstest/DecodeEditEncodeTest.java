package com.zhh.zhhcamera.ctstest;
...
//         th.join();
   ...
   private void videoEditTest() throws IOException {
      ...
      if (DEBUG_SAVE_FILE) {
         ...
         String dirName = Utils.getApp().getExternalCacheDir().getAbsolutePath();
      ...
      while (!outputDone) {
         ...
         if (!inputDone) {
            if (generateIndex == NUM_FRAMES) {
               ...
               long time = computePresentationTime(generateIndex) * 1000;
   ...
   private void assertTrue(boolean decoderConfigured) {
      assert  decoderConfigured;
   }

   private void assertFalse(boolean decoderConfigured) {
      assert  !decoderConfigured;
   }

   private void assertEquals(String wrong_time_stamp, long computePresentationTime, long presentationTimeUs) {
      assert computePresentationTime == presentationTimeUs;
   }

   private void assertEquals(long computePresentationTime, long presentationTimeUs) {
      assert computePresentationTime == presentationTimeUs;
   }
}
